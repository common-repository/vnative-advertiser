...
class vnad_Options {
    ...
    public function getTrackingLastSend() {
        return $this->getOption('TrackingLastSend['.vnad_PLUGIN_SLUG.']', 0);
    }
    public function setTrackingLastSend($value) {
        $this->setOption('TrackingLastSend['.vnad_PLUGIN_SLUG.']', $value);
    }
    public function getPluginInstallDate() {
        return $this->getOption('PluginInstallDate['.vnad_PLUGIN_SLUG.']', 0);
    }
    public function setPluginInstallDate($value) {
        $this->setOption('PluginInstallDate['.vnad_PLUGIN_SLUG.']', $value);
    }
    public function getPluginUpdateDate() {
        return $this->getOption('PluginUpdateDate['.vnad_PLUGIN_SLUG.']', 0);
    }
    public function setPluginUpdateDate($value) {
        $this->setOption('PluginUpdateDate['.vnad_PLUGIN_SLUG.']', $value);
    }
    ...
    public function isLoggerEnable() {
        return ($this->getOption('LoggerEnable', FALSE) || (defined('vnad_LOGGER') && vnad_LOGGER));
    }
    ...
    public function pushConversionSnippets($options, vnad_EcommercePurchase $purchase) {
        global $vnad;
        $this->setRequest('EcommercePurchase', $purchase);
        $snippets=$vnad->Manager->getConversionSnippets($options);
        foreach($snippets as $v) {
            $id=$v['id'];
            $vnad->Options->pushConversionSnippetId($id);
        }
    }
    ...
    public function getEcommercePurchase() {
        /* @var $result vnad_EcommercePurchase */
    ...
    private function pushGenericMessage($type, $message, $v1=NULL, $v2=NULL, $v3=NULL, $v4=NULL, $v5=NULL) {
        ...
        $array[]=$vnad->Lang->L($message, $v1, $v2, $v3, $v4, $v5);
    ...
    private function writeGenericMessages($type, $clean=TRUE) {
        ...
        if(is_array($array) && count($array)>0) {
            $result=TRUE;
            ?>
            <div class="vnad-box-<?php echo strtolower($type)?>"><?php echo wpautop(implode("\n", $array)); ?></div>
        <?php }
        if($clean) {
