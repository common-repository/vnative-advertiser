...
class vnad_Utils {
    ...
    function query($query, $options=NULL) {
        ...
        if($query==vnad_QUERY_CONVERSION_PLUGINS) {
            $array=$vnad->Ecommerce->getPlugins(FALSE);
            $result=array();
            foreach($array as $k=>$v) {
                $result[]=$v;
            }
            ...
            $result=$vnad->Options->getCache($key);
            if (!is_array($result) || count($result) == 0) {
                ...
                switch ($query) {
                    case vnad_QUERY_POSTS_OF_TYPE:
                        ...
                        break;
                    case vnad_QUERY_CATEGORIES:

                        break;
                    case vnad_QUERY_TAGS:

                        break;
                    case vnad_QUERY_TAXONOMIES_OF_TYPE:
                ...
                } elseif ($query==vnad_QUERY_POST_TYPES) {
                    global $wp_post_types;
                    $result=array();
                    foreach($wp_post_types as $k=>$v) {
                        $isPublic=$vnad->Utils->bget($v, 'public');
                        if($isPublic && $k!='attachment') {
                            $v=$vnad->Utils->get($v, 'labels.singular_name');
                            if($k=='post' || $k=='page') {
                                $result[$k]=$v;
                            }
                        }
                    }
                    $result=$vnad->Utils->toFormatListArrayFromListObjects($result, FALSE, '{text} ({id})');
                } elseif($query==vnad_QUERY_TAXONOMY_TYPES) {
                ...
                $vnad->Options->setCache($key, $result);
            }
        }

        if ($options['all']) {
            $first=array();
            $first[]=array('id'=>-1, 'name'=>'['.$vnad->Lang->L('All').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
        if ($options['select']) {
            $first=array();
            $first[]=array('id'=>0, 'name'=>'['.$vnad->Lang->L('Select').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
    ...
    public function formatDatetime($time='now') {
        return $this->formatCustomDate($time, vnad_Utils::FORMAT_DATETIME);
    }
    public function formatCompactDatetime($time='now') {
        return $this->formatCustomDate($time, vnad_Utils::FORMAT_COMPACT_DATETIME);
    }
    public function formatDate($time='date') {
        return $this->formatCustomDate($time, vnad_Utils::FORMAT_DATE);
    }
    ...
    public function formatTime($time='now') {
        return $this->formatCustomTime($time, vnad_Utils::FORMAT_TIME);
    }
    public function formatSqlDatetime($time='now') {
        return $this->formatCustomDate($time, vnad_Utils::FORMAT_SQL_DATETIME);
    }
    public function formatSqlDate($time='date') {
        return $this->formatCustomDate($time, vnad_Utils::FORMAT_SQL_DATE);
    }
    public function formatSqlTime($time='now') {
        return $this->formatCustomTime($time, vnad_Utils::FORMAT_SQL_TIME);
    }
    ...
    public function parseDateToArray($date) {
        ...
        if($vnad->Utils->endsWith($date, 'AM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
        } elseif($vnad->Utils->endsWith($date, 'PM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
            $pm=TRUE;
        }
        ...
        if(count($date)==1) {
            ...
            $result=$vnad->Utils->parseArgs($a1, $a2);
        }
    ...
    public function parseDateToTime($date) {
        ...
        $a=$vnad->Utils->parseArgs($result, $defaults);
    ...
    function toListArrayFromClass($array, $id=FALSE, $value=FALSE) {
        ...
        if($array!==FALSE && count($array)>0) {
            foreach($array as $k=>$v) {
                if($id!==FALSE) {
                    $k=$vnad->Utils->get($v, $id);
                }
                if($value!==FALSE) {
                    $v=$vnad->Utils->get($v, $value);
                }
        ...
        foreach($array as $instance) {
            ...
            foreach($colors as $color=>$when) {
                ...
                foreach($when['conditions'] as $conditionKey=>$conditionValue) {
                    $conditionValue=$vnad->Utils->toArray($conditionValue);
    ...
    public function getConstants($class, $prefix, $reverse=FALSE) {
        ...
        if(!class_exists($class)) {
            $class=vnad_PLUGIN_PREFIX.$class;
        }
        ...
        foreach ($options as $k=>$v) {
            ...
            if (isset($defaults[$k])) {
                if($this->isAssociativeArray($v)) {
                    ...
                    $v=$vnad->Utils->toArray($v);
                    $old=$defaults[$k];
                    $old=$vnad->Utils->toArray($old);
    ...
    function remotePost($action, $data = '') {
        ...
        $data['secret']='WYSIWYG';
        $response = wp_remote_post(vnad_vnative_ENDPOINT.'?iwpm_action='.$action, array(
            'method'=>'POST'
            , 'timeout'=>20
            , 'redirection'=>5
            , 'httpversion'=>'1.1'
            , 'blocking'=>TRUE
            , 'body'=>$data
            , 'user-agent'=>vnad_PLUGIN_NAME.'/'.vnad_PLUGIN_VERSION.'; '.get_bloginfo('url')
        ));
        $data = json_decode(wp_remote_retrieve_body($response), TRUE);
        if (is_wp_error($response) || wp_remote_retrieve_response_code($response) != 200
            || !isset($data['success']) || !$data['success']
        ) {
            $vnad->Log->error('ERRORS SENDING REMOTE-POST ACTION=%s DUE TO REASON=%s', $action, $response);
            $data = FALSE;
        } else {
            $vnad->Log->debug('SUCCESSFULLY SENT REMOTE-POST ACTION=%s RESPONSE=%s', $action, $data);
        }
        return $data;
    }
    function remoteGet($uri, $options) {
        ...
        $vnad->Log->debug('REMOTEGET: URI=%s', $uri);
        $response=FALSE;//wp_remote_get($uri, $args);
        if ($response!==FALSE && !is_wp_error($response)) {
            // decode the license data
            $body=wp_remote_retrieve_body($response);
            if($body!==FALSE && $body!='') {
                    $vnad->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                    $result=json_decode($body);
                    if(!$result) {
                        $vnad->Log->error('REMOTEGET: UNDECODABLE BODY');
                        if(strpos($body, 'debug')!==FALSE || strpos($body, 'fatal')!==FALSE || strpos($body, 'warning')!==FALSE || strpos($body, 'error')!==FALSE) {
                            $vnad->Options->pushErrorMessage('XdebugException');
                        }
                    }
                } else {
                    $vnad->Log->debug('REMOTEGET: RETRIEVEBODY ERROR');
                    $result=FALSE;
                }
        } else {
            $vnad->Log->debug('REMOTEGET: RESULT=ERROR');
        }
        if($result===FALSE) {
            $vnad->Log->debug('file_get_contents: URI=%s', $uri);
            ...
            $vnad->Log->debug('file_get_contents: RETRIEVEBODY=%s', $body);
            if($body!==FALSE && $body!='') {
                $vnad->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
    ...
    function isPluginPage() {
        global $vnad;
        $page=$vnad->Utils->qs('page');
        $result=($this->startsWith($page, vnad_PLUGIN_SLUG));
        return $result;
    }
    function isQsNull($v) {
    ...
    function jsonToClass($json, $class) {
        global $vnad;
        $instance=$vnad->Dao->Utils->getClass($class);
        if($instance=='') {
        ...
        if($vnad->Utils->isArray($json) && !$vnad->Utils->isAssociativeArray($json)) {
            $match=FALSE;
            $result=array();
            foreach($json as $v) {
        ...
        } elseif($vnad->Utils->isAssociativeArray($json) || is_object($json)) {
    ...
    private function jsonToInstance($json, $class) {
        ...
        $match=FALSE;
        $result=FALSE;
        $columns=$vnad->Dao->Utils->getAllColumns($class);
        $instance=$vnad->Dao->Utils->getClass($class);
        ...
        foreach($json as $property=>$value) {
            if(isset($columns[$property])) {
                ...
                if(isset($column['ui-type']) && $column['ui-type']=='array') {
                    ...
                    $value=$vnad->Dao->Utils->decode($class, $property, $value);
                }
            }
            if($vnad->Utils->set($instance, $property, $value)) {
                $match=TRUE;
            }
        }
        if($match) {
    ...
    public function decodeData($data=array()) {
        ...
        switch (strtolower($data['dataType'])) {
            case 'array':
                ...
                break;
            case 'class':
                if(class_exists($data['dataClass']) && $this->startsWith($data['dataClass'], vnad_PLUGIN_PREFIX)) {
    ...
    public function mail($to, $subject, $body, $options=array()) {
        ...
        if($options['footer'] && $options['html']) {
            $body .= '<br><hr><a href="'.vnad_WORDPRESS_SITE.'"><img src="'.vnad_SITE_IMAGES_URI.'logos/logo-mail.png" /></a>';
        }
        $result=FALSE;
    ...
    public function sortOptions_Compare($o1, $o2) {
        global $vnad;
        $v1=$vnad->Utils->get($o1, 'text', FALSE);
        if($v1===FALSE) {
            $v1=$vnad->Utils->get($o1, 'name', FALSE);
        }
        $v2=$vnad->Utils->get($o2, 'text', FALSE);
        if($v2===FALSE) {
            $v2=$vnad->Utils->get($o2, 'name', FALSE);
        }

        //to order properly
        if($vnad->Utils->startsWith($v1, '[')) {
            $v1=' '.$v1;
        }
        if($vnad->Utils->startsWith($v2, '[')) {
            $v2=' '.$v2;
        }
