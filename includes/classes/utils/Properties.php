...
class vnad_Properties {
    ...
    private function encode($t, $v) {
        ...
        switch (strtolower($t)) {
            ...
            case 'f':
                ...
                break;
            case 'd':
                $v=$vnad->Utils->formatDatetime($v);
                break;
            case 'b':
                $v=($vnad->Utils->isTrue($v) ? 'true' : 'false');
                break;
            case 'a':
    ...
    private function decode($t, $v) {
        ...
        switch (strtolower($t)) {
            ...
            case 'f':
                ...
                break;
            case 'd':
                $v=$vnad->Utils->parseDateToTime($v);
                break;
            case 'b':
                $v=$vnad->Utils->isTrue($v);
                break;
            case 'a':
                $v=$vnad->Utils->toArray($v);
                break;
    ...
    public function getFile($key, $default=FALSE) {
        ...
        if(is_dir($file)) {
            ...
            $file=str_replace("/", DIRECTORY_SEPARATOR, $file);
            if(!$vnad->Utils->endsWith($file, DIRECTORY_SEPARATOR)) {
